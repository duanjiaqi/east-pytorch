...
def get_images_for_test(inpath):
    '''
...
def resize_image(im, max_side_len=640):
    '''
...
def predict(model, inpath, outpath):
    # prepare ooutput directory
    result_root = os.path.abspath(outpath)
    if not os.path.exists(result_root):
        os.makedirs(result_root)

    model = model.eval()
    im_fn_list = get_images_for_test(inpath)
    start = time.time()
    ...
    for idx, im_fn in enumerate(im_fn_list):
        ...
        label = transforms.ToPILImage()(score[0].detach().cpu()).convert('L')
        label.show()
        time.sleep(5)
        ...
        if boxes is not None:
            res_file = os.path.join(outpath, 'res_{}.txt'.format(os.path.basename(im_fn).split('.')[0]))
        ...
        img_path = os.path.join(outpath, os.path.basename(im_fn))
...
if __name__ == "__main__":
    weightpath = '/data/djq/github/east-pytorch/parameter/original/epoch_1600_checkpoint.pth.tar'
    ...
    inpath = '/data/djq/gitlab/text-det-pytorch/data/IC15/mutil-scale'
    outpath = '/data/djq/gitlab/text-det-pytorch/data/IC15/mutil-scale-out'
    predict(model_, inpath, outpath)
