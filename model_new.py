...
class East(nn.Module):
    ...
    def forward(self,images):
        ...
        F_score = self.sigmoid8(F_score)
        geo_map = self.conv9(g)
        geo_map = self.relu9(geo_map) * 4
        angle_map = self.conv10(g)
        angle_map = torch.where(angle_map < (math.pi / 4), angle_map, torch.ones_like(angle_map)*(math.pi / 4))
        angle_map = torch.where(angle_map < (-math.pi / 4), torch.ones_like(angle_map)*(-math.pi / 4), angle_map)
