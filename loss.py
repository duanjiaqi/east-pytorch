...
class LossFunc(nn.Module):
    ...
    def forward(self, y_true_cls, y_pred_cls, y_true_geo, y_pred_geo, training_mask, step):
        classification_loss = dice_coefficient(y_true_cls, y_pred_cls, training_mask)
        # scale classification loss to match the iou loss part
        log.writer.add_scalar('classification/step', classification_loss.item(), step)
    ...
    #     d1_pred, d2_pred, d3_pred, d4_pred, theta_pred = tf.split(value=y_pred_geo, num_or_size_splits=5, axis=3)
        ...
        log.writer.add_scalar('L_AABB/step', torch.mean(L_AABB * y_true_cls * training_mask).item(), step)
        log.writer.add_scalar('L_theta/step', torch.mean(L_theta * y_true_cls * training_mask).item(), step)
