...
def poly_short_side(poly):
    dist1 = np.linalg.norm(poly[0] - poly[1])
    dist2 = np.linalg.norm(poly[1] - poly[2])
    dist3 = np.linalg.norm(poly[2] - poly[3])
    dist4 = np.linalg.norm(poly[3] - poly[0])
...
def predict(model, epoch):
    # max_lens = [(640,0.1,0.1, (30, 150)), (1280,0.1,0.1, (30, 70)), (2560,0.1,0.1, (15, 70))]
    max_lens = [(1280,0.1,0.1, (50, 120)), (2560,0.1,0.1, (15, 120))]
    # max_lens = [(2560,0.1,0.1, (0, 600))]
    # prepare ooutput directory
    result_root = cfg.parameter_path
    ...
    for idx, im_fn in enumerate(im_fn_list):
        ...
        for max_len in max_lens:
            ...
            score = torch.nn.Softmax(dim=1)(score)[:,1:,:,:]
        ...
        print('\r', str(idx) + 'images done', end='', flush=True)

    command = "cd %s && zip res.zip *txt" % output_dir_txt
    os.system(command)
    gt_name = 'gt.zip'
    ...
    command = '/mnt/lustre/duanjiaqi/anaconda3/envs/py2/bin/python ../data/ic15eval_script/script.py -s={submit_zip_path} -g={gt}'.format(submit_zip_path=output_dir_txt + '/res.zip', gt=gt_path+gt_name)
